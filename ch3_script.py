...
print(f"固定スキームのMSFE: {np.mean(sqe_result1_list)}")
...
print(f"再帰的スキームのMSFE: {np.mean(sqe_result2_list)}")
...
print(f"ローリングスキームのMSFE: {np.mean(sqe_result3_list)}")
...
ax1.plot(dti[H + PERIOD :], np.ravel(predict_result1_list), label="固定スキーム", color="black")
ax1.plot(dti[H + PERIOD :], np.ravel(predict_result2_list), label="再帰的スキーム", color="grey", marker="x")
ax1.plot(dti[H + PERIOD :], np.ravel(predict_result3_list), label="ローリングスキーム", color="black", linestyle="dashed")
ax1.legend(loc="lower left", fontsize=16)
ax1.tick_params(labelsize=16)
ax1.set_ylabel("％", fontsize=16)
ax2.set_title("(b)2乗予測誤差", fontsize=24)
ax2.plot(dti[H + PERIOD :], np.ravel(sqe_result1_list), label="固定スキーム", color="black")
ax2.plot(dti[H + PERIOD :], np.ravel(sqe_result2_list), label="再帰的スキーム", color="grey", marker="x")
ax2.plot(dti[H + PERIOD :], np.ravel(sqe_result3_list), label="ローリングスキーム", color="black", linestyle="dashed")
...
#####
# AR(p)モデルによる外挿予測
scheme = str(input("スキームを選択してください(固定、再帰的、ローリングのいずれか): "))
...
for k in range(1, qdlen - PERIOD):
    ...
    # 四半期の2か月目のときは、1か月先をAR(1)予測したうえで、ブリッジモデルを作成
    ...
    # 四半期の1か月目のときは、1か月先と2か月先をAR(1)予測したうえで、ブリッジモデルを作成
...
for k in range(1, qdlen - PERIOD):
    ...
    # 四半期の3か月目のときは、当四半期の1か月目、2か月目、3か月目のデータからMIDASモデルを作成
    ...
    # 四半期の2か月目のときは、前四半期の3か月目、当四半期の1か月目、2か月目のデータからMIDASモデルを作成
    y = np.ravel(data_q[k : k + PERIOD].to_numpy())
    x11 = data_m[3 * k : 3 * (k + PERIOD) : 3].iloc[:, 0].to_numpy()
    x12 = data_m[3 * k + 1 : 3 * (k + PERIOD) + 1 : 3].iloc[:, 0].to_numpy()
    ...
    # 四半期の1か月目のときは、前四半期の2か月目、3か月目、当四半期の1か月目のデータからMIDASモデルを作成
    y = np.ravel(data_q[k : k + PERIOD].to_numpy())
    x11 = data_m[3 * k : 3 * (k + PERIOD) : 3].iloc[:, 0].to_numpy()
...
predict_result = np.zeros(((qdlen - 1) - PERIOD) * 3)
for k in range(1, qdlen - PERIOD):
    # 四半期の3か月目のときは、当四半期の1か月目、2か月目、3か月目のデータからU-MIDASモデルを作成
    ...
    # 四半期の2か月目のときは、前四半期の3か月目、当四半期の1か月目、2か月目のデータからU-MIDASモデルを作成
    y = pd.DataFrame(data_q[k : k + PERIOD].to_numpy())
    x11 = data_m[3 * k : 3 * (k + PERIOD) : 3].iloc[:, 0].to_numpy()
    x12 = data_m[3 * k + 1 : 3 * (k + PERIOD) + 1 : 3].iloc[:, 0].to_numpy()
    x13 = data_m[3 * k - 1 : 3 * (k + PERIOD) - 1 : 3].iloc[:, 0].to_numpy()
    ...
    # 四半期の3か月目のときは、前四半期の2か月目、3か月目、当四半期の1か月目のデータからU-MIDASモデルを作成
    y = pd.DataFrame(data_q[k : k + PERIOD].to_numpy())
    x11 = data_m[3 * k : 3 * (k + PERIOD) : 3].iloc[:, 0].to_numpy()
    x12 = data_m[3 * k - 2 : 3 * (k + PERIOD) - 2 : 3].iloc[:, 0].to_numpy()
    x13 = data_m[3 * k - 1 : 3 * (k + PERIOD) - 1 : 3].iloc[:, 0].to_numpy()
...
ax1.plot(qdti[PERIOD + 1 :], np.ravel(predict_result1_list) * 100, label="ブリッジモデル（対象四半期の1か月目まで反映）", color="black")
ax1.plot(qdti[PERIOD + 1 :], np.ravel(predict_result2_list) * 100, label="ブリッジモデル（対象四半期の2か月目まで反映）", color="grey", marker="x")
ax1.plot(qdti[PERIOD + 1 :], np.ravel(predict_result3_list) * 100, label="ブリッジモデル（対象四半期の3か月目まで反映）", color="black", linestyle="dashed")
...
ax2.plot(qdti[PERIOD + 1 :], np.ravel(predict_result4_list) * 100, label="MIDAS（対象四半期の1か月目まで反映）", color="black")
ax2.plot(qdti[PERIOD + 1 :], np.ravel(predict_result5_list) * 100, label="MIDAS（対象四半期の2か月目まで反映）", color="grey", marker="x")
ax2.plot(qdti[PERIOD + 1 :], np.ravel(predict_result6_list) * 100, label="MIDAS（対象四半期の3か月目まで反映）", color="black", linestyle="dashed")
...
ax3.plot(qdti[PERIOD + 1 :], np.ravel(predict_result7_list) * 100, label="U-MIDAS（対象四半期の1か月目まで反映）", color="black")
ax3.plot(qdti[PERIOD + 1 :], np.ravel(predict_result8_list) * 100, label="U-MIDAS（対象四半期の2か月目まで反映）", color="grey", marker="x")
ax3.plot(qdti[PERIOD + 1 :], np.ravel(predict_result9_list) * 100, label="U-MIDAS（対象四半期の3か月目まで反映）", color="black", linestyle="dashed")
...
# Granger and Ramanathan (1984)の合成予測
def compute_gr(pack):
...
for i in range((nsample - H) - PERIOD):
    ...
    fitted1 = model.predict(train_x1)
    fitted2 = model.predict(train_x2)
    fitted3 = model.predict(train_x3)
    fitted4 = model.predict(train_x4)
    fitted5 = model.predict(train_x5)
    fitted6 = model.predict(train_x6)
    ...
    forecast1 = model.predict(test_x1)
    forecast2 = model.predict(test_x2)
    forecast3 = model.predict(test_x3)
    forecast4 = model.predict(test_x4)
    forecast5 = model.predict(test_x5)
    forecast6 = model.predict(test_x6)
    pack_test = pd.concat(
        [
...
ax2.plot(dti[H + PERIOD :], np.ravel(am_list), label="算術平均", color="black")
ax2.plot(dti[H + PERIOD :], np.ravel(bg_list), label="Bates and Granger (1969)の合成予測", color="grey", marker="x")
ax2.plot(dti[H + PERIOD :], np.ravel(gr_list), label="Granger and Ramanathan (1984)の合成予測", color="black", linestyle="dashed")
ax2.legend(loc="lower left", fontsize=16)
ax2.tick_params(labelsize=16)
ax2.set_ylabel("％", fontsize=16)
ax3.set_title("(c)累積2乗予測誤差", fontsize=24)
ax3.plot(dti[H + PERIOD :], ((caogap[H+PERIOD:]-np.ravel(am_list))**2).cumsum(), label="算術平均", color='black')
ax3.plot(dti[H + PERIOD :], ((caogap[H+PERIOD:]-np.ravel(bg_list))**2).cumsum(), label="Bates and Granger (1969)の合成予測", color="grey", marker="x")
ax3.plot(dti[H + PERIOD :], ((caogap[H+PERIOD:]-np.ravel(gr_list))**2).cumsum(),label="Granger and Ramanathan (1984)の合成予測", color="black", linestyle="dashed")
