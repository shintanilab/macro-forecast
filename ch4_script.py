...
for i in range((vsample - H) - PERIOD):
    ...
    for npc in range(1, MAX_K + 1 + 1):
        ...
        ahols = LinearRegression()
        ahols.fit(df_pca_train, train_x_std)
        fitted = ahols.predict(df_pca_train)
        ah_ssr = (((train_x_std - fitted) ** 2).sum(axis=0)).sum(axis=0)
        ah_tss = ((train_x_std**2).sum(axis=0)).sum(axis=0)
        trace = 1 - ah_ssr / ah_tss
...
ax1.plot(dti[H + PERIOD :], msdata.x204[H + PERIOD :], label="実績値", color="black", linestyle="dashed")
ax1.plot(dti[H + PERIOD :], pd.DataFrame(predict_result_list).to_numpy(), label="動学因子モデルによる予測値", color="black")
ax1.legend(loc="upper right", fontsize=16)
ax1.set_ylabel("％", fontsize=16)
ax1.tick_params(labelsize=16)
ax2.set_title("(b)動学因子モデルにおける共通因子数", fontsize=24)
ax2.plot(dti[H + PERIOD :], ah_result_list, label="Ahn and Horenstein (2013)に基づき選択された共通因子", color="black")
ax2.legend(loc="upper right", fontsize=16)
ax2.tick_params(labelsize=16)
ax2.set_yticks(np.arange(0, MAX_K + 1, step=2))
...
for k in range(1, qdlen - PERIOD):
    ...
    # 四半期の3か月目のときは、当四半期の1か月目、2か月目、3か月目のデータからMIDASモデルを構築
    ...
    # 四半期の2か月目のときは、前四半期の3か月目、当四半期の1か月目、2か月目のデータからMIDASモデルを構築
    data_y = pd.DataFrame(data_q[k : k + PERIOD].to_numpy())
    ...
    # 四半期の1か月目のときは、前四半期の2か月目、3か月目、当四半期の1か月目のデータからMIDASモデルを構築
    data_y = pd.DataFrame(data_q[k : k + PERIOD].to_numpy())
...
predict_result = np.zeros(((qdlen - 1) - PERIOD) * 3)
for k in range(1, qdlen - PERIOD):
    # 四半期の3か月目のときは、当四半期の1か月目、2か月目、3か月目のデータからU-MIDASモデルを構築
    ...
    # 四半期の2か月目のときは、前四半期の3か月目、当四半期の1か月目、2か月目のデータからU-MIDASモデルを構築
    y = pd.DataFrame(data_q[k : k + PERIOD].to_numpy())
    x2 = data_m[3 * k - 1 : 3 * (k + PERIOD + 1) - 1].to_numpy()
    scaler = StandardScaler()
    scaler.fit(x2)
...
ax1.plot(qdti[PERIOD + 1 :], np.ravel(predict_result1_list), label="因子ブリッジモデル（対象四半期の1か月目まで反映）", color="black")
ax1.plot(qdti[PERIOD + 1 :], np.ravel(predict_result2_list), label="因子ブリッジモデル（対象四半期の2か月目まで反映）", color="grey")
ax1.plot(qdti[PERIOD + 1 :], np.ravel(predict_result3_list), label="因子ブリッジモデル（対象四半期の3か月目まで反映）", color="black", linestyle="dashed")
...
ax2.plot(qdti[PERIOD + 1 :], np.ravel(predict_result4_list), label="因子MIDAS（対象四半期の1か月目まで反映）", color="black")
ax2.plot(qdti[PERIOD + 1 :], np.ravel(predict_result5_list), label="因子MIDAS（対象四半期の2か月目まで反映）", color="grey")
ax2.plot(qdti[PERIOD + 1 :], np.ravel(predict_result6_list), label="因子MIDAS（対象四半期の3か月目まで反映）", color="black", linestyle="dashed")
...
ax3.plot(qdti[PERIOD + 1 :], np.ravel(predict_result7_list), label="因子U-MIDAS（対象四半期の1か月目まで反映）", color="black")
ax3.plot(qdti[PERIOD + 1 :], np.ravel(predict_result8_list), label="因子U-MIDAS（対象四半期の2か月目まで反映）", color="black")
ax3.plot(qdti[PERIOD + 1 :], np.ravel(predict_result9_list), label="因子U-MIDAS（対象四半期の3か月目まで反映）", color="black", linestyle="dashed")
