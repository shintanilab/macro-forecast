...
ax1.plot(dti[H + PERIOD :], np.ravel(predict_result_list), label="動学因子モデル", color="black")
ax1.plot(dti[H + PERIOD :], np.ravel(predict_resultAR_list), label="ARモデル", color="grey", marker="x")
ax1.legend(loc="upper left", fontsize=16)
ax1.set_ylabel("％", fontsize=16)
ax1.tick_params(labelsize=16)
ax2.set_title("(b)累積2乗予測誤差", fontsize=24)
ax2.plot(dti[H + PERIOD :], np.ravel(sqe_result_list).cumsum(), label="動学因子モデル", color="black")
ax2.plot(dti[H + PERIOD :], np.ravel(sqe_resultAR_list).cumsum(), label="ARモデル", color="grey", marker="x")
